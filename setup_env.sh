...
# === 记录脚本所在目录 ===
EASIM_ENV_SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
echo "一键安装环境脚本所在目录: ${EASIM_ENV_SCRIPT_DIR}"

# === 配置参数 ===
ISAACSIM_VERSION="v5.1.0"
ISAACLAB_VERSION="v2.3.0"
WORKSPACE="${HOME}/isaac_workspace"
PYTHON_VERSION="3.11"
...
conda remove -n env_isaaclab_easim --all -y || true
...
cd ${EASIM_ENV_SCRIPT_DIR}
echo "当前目录: $(pwd)"
if [ -L "isaac_workspace" ] || [ -e "isaac_workspace" ]; then
  rm -f isaac_workspace
fi
ln -s ${WORKSPACE} isaac_workspace
echo "✅ ${WORKSPACE} 已软链接到 ${EASIM_ENV_SCRIPT_DIR}/isaac_workspace"

# === 安装easim包到IsaacLab环境 ===
echo "[额外步骤2/2] 将 easim 安装到 IsaacLab 环境..."
eval "$(conda shell.bash hook)"
conda activate ${ENV_NAME}
cd ${EASIM_ENV_SCRIPT_DIR}
python3 -m pip install -e .
echo "✅ easim 已安装到 ${ENV_NAME} 环境"
...
echo "  ./isaac_workspace/IsaacLab/isaaclab.sh -p ${EASIM_ENV_SCRIPT_DIR}/isaac_workspace/IsaacLab/scripts/tutorials/00_sim/create_empty.py"
echo ""
echo "运行 easim 自定义任务:"
echo "  conda activate ${ENV_NAME}"
echo "  ./isaac_workspace/IsaacLab/isaaclab.sh -p ${EASIM_ENV_SCRIPT_DIR}/isaac_workspace/source/easim/teleop/run_teleop.py --task Isaac-PickPlace-G1-InspireFTP-Abs-v0-Teleop --teleop_device handtracking --enable_pinocchio"
echo ""
