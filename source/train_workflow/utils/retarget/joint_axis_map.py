...
约定：
- 肩部 (左/右 shoulder_*) 走 ``unitree_g1.compute_shoulder_angles``：
...
- 腿部：
  - hip (左/右 hip_*) 走 ``unitree_g1.compute_hip_angles``，axis_idx: **0=pitch, 1=roll, 2=yaw**；
  - ankle (左/右 ankle_*) 走 ``unitree_g1.compute_ankle_angles``，axis_idx: **0=pitch, 1=roll**。
- 腰部 (waist_*) 走 ``unitree_g1.compute_waist_angles``（外旋 szxy + 物理轴→语义重排）：
...
- 肘 (左/右 elbow) 走 ``unitree_g1.compute_elbow_angle``：取前臂长轴在肘骨旋转
  前后的夹角作为铰链弯曲角（自动剔除被烘焙进肘骨的前臂自转 pronation）。
  axis_idx 在此分支不再使用；scale 仅作 ±1 sign（两肘默认 -1，均朝负向弯）。
- 腕 (左/右 wrist) 走 ``unitree_g1.compute_wrist_angles``（与肩同架构：
...
G1_JOINT_AXIS_MAP_RAW: dict[str, AxisMapRawEntry] = {
    # 手臂：肩+腕 组合 -> unitree_g1.compute_shoulder_angles -> (pitch, roll, yaw)
    ...
    # 躯干：上半身+上半身2 -> unitree_g1.compute_waist_angles -> (pitch, roll, yaw)
