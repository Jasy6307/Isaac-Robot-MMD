...
from source.train_workflow.utils.format.hdf5 import infer_hdf5_has_hand_data, load_hdf5_motion
...
def resolve_path_under_media(relative: str, media_dir: str) -> str:
    """Resolve config-relative path under ``media/``."""
    ...
    *,
    ...
    for i, ent in enumerate(items):
        ...
        if raw_key is None or str(raw_key).strip() == "":
            from source.train_workflow.utils.motion.sync import ui_only_dance_key
