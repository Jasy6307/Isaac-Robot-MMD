...
:: === 记录脚本所在目录（去掉末尾反斜杠）===
set "EASIM_ENV_SCRIPT_DIR=%~dp0"
if "%EASIM_ENV_SCRIPT_DIR:~-1%"=="\" set "EASIM_ENV_SCRIPT_DIR=%EASIM_ENV_SCRIPT_DIR:~0,-1%"
echo 一键安装环境脚本所在目录: %EASIM_ENV_SCRIPT_DIR%

:: === 配置参数 ===
set "ISAACSIM_VERSION=v5.1.0"
set "ISAACLAB_VERSION=v2.3.0"
set "WORKSPACE=I:\isaac"
...
:: 检查 I:\isaac 是否已有同名 zip 或目录，有则跳过下载
set "SKIP_DOWNLOAD=0"
...
cd /d "%EASIM_ENV_SCRIPT_DIR%"
if exist "isaac_workspace" (
    rmdir "isaac_workspace" 2>nul || del /q "isaac_workspace" 2>nul
)
mklink /J isaac_workspace "%WORKSPACE%"
echo %WORKSPACE% 已联接到 %EASIM_ENV_SCRIPT_DIR%\isaac_workspace

:: === 安装 easim 包到 Isaac Lab 环境 ===
echo [额外步骤2/2] 将 easim 安装到 Isaac Lab 环境...
...
if exist "setup.py" (
    ...
    echo 注意: 当前目录无 setup.py 或 pyproject.toml，跳过 easim 安装
)
...
echo 运行 easim 自定义任务:
echo   conda activate %ENV_NAME%
echo   %WORKSPACE%\IsaacLab\isaaclab.bat -p %EASIM_ENV_SCRIPT_DIR%\isaac_workspace\source\easim\teleop\run_teleop.py --task Isaac-PickPlace-G1-InspireFTP-Abs-v0-Teleop --teleop_device handtracking --enable_pinocchio
echo.
pause
